...
/*
 ...
 - 2017/08/25 (1.52) - io.MousePos needs to be set to float2(-FLT_MAX,-FLT_MAX) when mouse is unavailable/missing. Previously float2(-1,-1) was enough but we now accept negative mouse coordinates. In your binding if you need to support unavailable mouse, make sure to replace "io.MousePos = float2(-1,-1)" with "io.MousePos = float2(-FLT_MAX,-FLT_MAX)".
 - 2017/08/22 (1.51) - renamed IsItemHoveredRect() to IsItemRectHovered(). Kept inline redirection function (will obsolete). -> (1.52) use IsItemHovered(ImGuiHoveredFlags_RectOnly)!
                     - renamed IsMouseHoveringAnyWindow() to IsAnyWindowHovered() for consistency. Kept inline redirection function (will obsolete).
                     - renamed IsMouseHoveringWindow() to IsWindowRectHovered() for consistency. Kept inline redirection function (will obsolete).
 - 2017/08/20 (1.51) - renamed GetStyleColName() to GetStyleColorName() for consistency.
 - 2017/08/20 (1.51) - added PushStyleColor(ImGuiCol idx, uint32_t col) overload, which _might_ cause an "ambiguous call" compilation error if you are using ImColor() with implicit cast. Cast to uint32_t or float4 explicily to fix.
 ...
 - 2017/08/08 (1.51) - removed ColorEditMode() and ImGuiColorEditMode in favor of ImGuiColorEditFlags and parameters to the various Color*() functions. The SetColorEditOptions() allows to initialize default but the user can still change them with right-click context menu.
                     ...
                     - changed prototype of rarely used 'ColorButton(float4 col, bool small_height = false, bool outline_border = true)' to 'ColorButton(const char* desc_id, float4 col, ImGuiColorEditFlags flags = 0, float2 size = float2(0,0))'
 - 2017/07/20 (1.51) - removed IsPosHoveringAnyWindow(float2), which was partly broken and misleading. ASSERT + redirect user to io.WantCaptureMouse
 ...
 - 2016/09/25 (1.50) - style.WindowTitleAlign is now a float2 (ImGuiAlign enum was removed). set to (0.5f,0.5f) for horizontal+vertical centering, (0.0f,0.0f) for upper-left, etc.
 ...
 - 2016/05/12 (1.49) - title bar (using ImGuiCol_TitleBg/ImGuiCol_TitleBgActive colors) isn't rendered over a window background (ImGuiCol_WindowBg color) anymore.
                       ...
                       This helper function will convert an old TitleBg/TitleBgActive color into a new one with the same visual output, given the OLD color and the OLD WindowBg color.
                           float4 ConvertTitleBgCol(const float4& win_bg_col, const float4& title_bg_col)
                           {
                               float new_a = 1.0f - ((1.0f - win_bg_col.w) * (1.0f - title_bg_col.w)), k = title_bg_col.w / new_a;
                               return float4((win_bg_col.x * win_bg_col.w + title_bg_col.x) * k, (win_bg_col.y * win_bg_col.w + title_bg_col.y) * k, (win_bg_col.z * win_bg_col.w + title_bg_col.z) * k, new_a);
                           }
 ...
 - 2016/04/26 (1.49) - changed ImDrawList::PushClipRect(float4 rect) to ImDrawList::PushClipRect(Imvec2 min,float2 max,bool intersect_with_current_clip_rect=false). Note that higher-level ImGui::PushClipRect() is preferable because it will clip at logic/widget level, whereas ImDrawList::PushClipRect() only affect your renderer.
...
static void             SetWindowPos(ImGuiWindow* window, const float2& pos, ImGuiCond cond);
static void             SetWindowSize(ImGuiWindow* window, const float2& size, ImGuiCond cond);
static void             SetWindowCollapsed(ImGuiWindow* window, bool collapsed, ImGuiCond cond);
static void             FindHoveredWindow();
static ImGuiWindow*     CreateNewWindow(const char* name, float2 size, ImGuiWindowFlags flags);
...
namespace ImGui
{
static bool             BeginChildEx(const char* name, ImGuiID id, const float2& size_arg, bool border, ImGuiWindowFlags flags);
...
static float2           NavCalcPreferredRefPos();
static void             NavSaveLastChildNavWindow(ImGuiWindow* nav_window);
static ImGuiWindow*     NavRestoreLastChildNavWindow(ImGuiWindow* window);

static void             UpdateMouseInputs();
static void             UpdateMouseWheel();
static void             UpdateManualResize(ImGuiWindow* window, const float2& size_auto_fit, int* border_held, int resize_grip_count, uint32_t resize_grip_col[4]);
}
...
//GUI Context
ImGuiContext::ImGuiContext(ImFontAtlas* shared_font_atlas) : OverlayDrawList(NULL)
{
	Initialized = false;
	...
	ActiveIdClickOffset = float2(-1,-1);
...
ImGuiStyle::ImGuiStyle()
{
    ...
    WindowPadding           = float2(8,8);      // Padding within a window
    WindowRounding          = 7.0f;             // Radius of window corners rounding. Set to 0.0f to have rectangular windows
    WindowBorderSize        = 1.0f;             // Thickness of border around windows. Generally set to 0.0f or 1.0f. Other values not well tested.
    WindowMinSize           = float2(32,32);    // Minimum window size
    WindowTitleAlign        = float2(0.0f,0.5f);// Alignment for title bar text
    ChildRounding           = 0.0f;             // Radius of child window corners rounding. Set to 0.0f to have rectangular child windows
    ChildBorderSize         = 1.0f;             // Thickness of border around child windows. Generally set to 0.0f or 1.0f. Other values not well tested.
    PopupRounding           = 0.0f;             // Radius of popup window corners rounding. Set to 0.0f to have rectangular child windows
    PopupBorderSize         = 1.0f;             // Thickness of border around popup or tooltip windows. Generally set to 0.0f or 1.0f. Other values not well tested.
    FramePadding            = float2(4,3);      // Padding within a framed rectangle (used by most widgets)
    FrameRounding           = 0.0f;             // Radius of frame corners rounding. Set to 0.0f to have rectangular frames (used by most widgets).
    FrameBorderSize         = 0.0f;             // Thickness of border around frames. Generally set to 0.0f or 1.0f. Other values not well tested.
    ItemSpacing             = float2(8,4);      // Horizontal and vertical spacing between widgets/lines
    ItemInnerSpacing        = float2(4,4);      // Horizontal and vertical spacing between within elements of a composed widget (e.g. a slider and its label)
    TouchExtraPadding       = float2(0,0);      // Expand reactive bounding box for touch-based system where touch position is not accurate enough. Unfortunately we don't sort widgets so priority on overlap will always be given to the first widget. So don't grow this too much!
    ...
    ButtonTextAlign         = float2(0.5f,0.5f);// Alignment of button text when button is larger than text.
    DisplayWindowPadding    = float2(20,20);    // Window positions are clamped to be visible within the display area by at least this amount. Only covers regular windows.
    DisplaySafeAreaPadding  = float2(3,3);      // If you cannot see the edge of your screen (e.g. on a TV) increase the safe area padding. Covers popups/tooltips as well regular windows.
...
ImGuiIO::ImGuiIO()
{
    ...
    DisplayFramebufferScale = float2(1.0f, 1.0f);
    DisplayVisibleMin = DisplayVisibleMax = float2(0.0f, 0.0f);
    ...
    MousePos = float2(-FLT_MAX, -FLT_MAX);
    MousePosPrev = float2(-FLT_MAX, -FLT_MAX);
...
float2 ImLineClosestPoint(const float2& a, const float2& b, const float2& p)
{
    float2 ap = p - a;
    float2 ab_dir = b - a;
...
bool ImTriangleContainsPoint(const float2& a, const float2& b, const float2& c, const float2& p)
{
...
void ImTriangleBarycentricCoords(const float2& a, const float2& b, const float2& c, const float2& p, float& out_u, float& out_v, float& out_w)
{
    float2 v0 = b - a;
    float2 v1 = c - a;
    float2 v2 = p - a;
...
float2 ImTriangleClosestPoint(const float2& a, const float2& b, const float2& c, const float2& p)
{
    float2 proj_ab = ImLineClosestPoint(a, b, p);
    float2 proj_bc = ImLineClosestPoint(b, c, p);
    float2 proj_ca = ImLineClosestPoint(c, a, p);
...
uint32_t ImHash(const void* data, int data_size, uint32_t seed)
{
...
float4 ImGui::ColorConvertU32ToFloat4(uint32_t in)
{
    float s = 1.0f/255.0f;
    return float4(
...
uint32_t ImGui::ColorConvertFloat4ToU32(const float4& in)
{
    uint32_t out;
...
uint32_t ImGui::GetColorU32(ImGuiCol idx, float alpha_mul)
{
    ...
    float4 c = style.Colors[idx];
    c.w *= style.Alpha * alpha_mul;
    return ColorConvertFloat4ToU32(c);
}

uint32_t ImGui::GetColorU32(const float4& col)
{
    ImGuiStyle& style = GImGui->Style;
    float4 c = col;
    c.w *= style.Alpha;
...
const float4& ImGui::GetStyleColorVec4(ImGuiCol idx)
{
    ImGuiStyle& style = GImGui->Style;
    return style.Colors[idx];
}

uint32_t ImGui::GetColorU32(uint32_t col)
{
    float style_alpha = GImGui->Style.Alpha;
    if (style_alpha >= 1.0f)
        return col;
    uint32_t a = (col & IM_COL32_A_MASK) >> IM_COL32_A_SHIFT;
    a = (uint32_t)(a * style_alpha); // We don't need to clamp 0..255 because Style.Alpha is in 0..1 range.
    ...
    : DrawListInst(&context->DrawListSharedData)
{
    ...
    Size = SizeFull = float2(0.0f, 0.0f);
    SizeContents = SizeContentsExplicit = float2(0.0f, 0.0f);
    WindowPadding = float2(0.0f, 0.0f);
    WindowRounding = 0.0f;
    WindowBorderSize = 0.0f;
    MoveId = GetID("#MOVE");
    ChildId = 0;
    Scroll = float2(0.0f, 0.0f);
    ScrollTarget = float2(FLT_MAX, FLT_MAX);
    ScrollTargetCenterRatio = float2(0.5f, 0.5f);
    ScrollbarSizes = float2(0.0f, 0.0f);
    ...
    SetWindowPosVal = SetWindowPosPivot = float2(FLT_MAX, FLT_MAX);
...
void ImGui::ItemSize(const float2& size, float text_offset_y)
{
    ImGuiContext& g = *GImGui;
    ...
    //if (g.IO.KeyAlt) window->DrawList->AddRect(window->DC.CursorPos, window->DC.CursorPos + float2(size.x, line_height), IM_COL32(255,0,0,200)); // [DEBUG]
    window->DC.CursorPosPrevLine = float2(window->DC.CursorPos.x + size.x, window->DC.CursorPos.y);
    window->DC.CursorPos = float2((float)(int)(window->Pos.x + window->DC.Indent.x + window->DC.ColumnsOffset.x), (float)(int)(window->DC.CursorPos.y + line_height + g.Style.ItemSpacing.y));
...
float2 ImGui::CalcItemSize(float2 size, float default_x, float default_y)
{
    ImGuiContext& g = *GImGui;
    float2 content_max;
...
float ImGui::CalcWrapWidthForPos(const float2& pos, float wrap_pos_x)
{
    if (wrap_pos_x < 0.0f)
        return 0.0f;
...
bool ImGui::DebugCheckVersionAndDataLayout(const char* version, size_t sz_io, size_t sz_style, size_t sz_vec2, size_t sz_vec4, size_t sz_vert)
{
    ...
    if (sz_vec2  != sizeof(float2))        { error = true; IM_ASSERT(sz_vec2  == sizeof(float2)       && "Mismatched struct layout!"); }
    if (sz_vec4  != sizeof(float4))        { error = true; IM_ASSERT(sz_vec4  == sizeof(float4)       && "Mismatched struct layout!"); }
...
void ImGui::UpdateMouseMovingWindow()
{
    ...
    if (g.MovingWindow != NULL)
    {
        ...
        {
            float2 pos = g.IO.MousePos - g.ActiveIdClickOffset;
...
static void ImGui::UpdateMouseInputs()
{
    ...
    if (IsMousePosValid(&g.IO.MousePos) && IsMousePosValid(&g.IO.MousePosPrev))
        ...
        g.IO.MouseDelta = float2(0.0f, 0.0f);
    ...
    for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++)
    {
        ...
        if (g.IO.MouseClicked[i])
        {
            ...
            {
                float2 delta_from_click_pos = IsMousePosValid(&g.IO.MousePos) ? (g.IO.MousePos - g.IO.MouseClickedPos[i]) : float2(0.0f, 0.0f);
            ...
            g.IO.MouseDragMaxDistanceAbs[i] = float2(0.0f, 0.0f);
...
void ImGui::UpdateMouseWheel()
{
    ...
    if (g.IO.MouseWheel != 0.0f)
    {
        if (g.IO.KeyCtrl && g.IO.FontAllowUserScaling)
        {
            ...
            const float2 offset = window->Size * (1.0f - scale) * (g.IO.MousePos - window->Pos) / window->Size;
            window->Pos += offset;
            window->Size *= scale;
...
void ImGui::NewFrame()
{
    ...
    g.DrawListSharedData.ClipRectFullscreen = float4(0.0f, 0.0f, g.IO.DisplaySize.x, g.IO.DisplaySize.y);
    ...
    g.PlatformImePos = float2(1.0f, 1.0f); // OS Input Method Editor showing on top-left of our window by default
    ...
    SetNextWindowSize(float2(400,400), ImGuiCond_FirstUseEver);
    Begin("Debug##Default");
}
...
void ImGui::Initialize(ImGuiContext* context)
{
	...
	// Todo: Antoine Verify results
    g.SettingsHandlers.insert(g.SettingsHandlers.begin(),ini_handler);
...
static void AddWindowToSortedBuffer(eastl::vector<ImGuiWindow*>* out_sorted_windows, ImGuiWindow* window)
{
    ...
    if (window->Active)
    {
        int count = (int)window->DC.ChildWindows.size();
        if (count > 1)
...
static void SetupDrawData(eastl::vector<ImDrawList*>* draw_lists, ImDrawData* out_draw_data)
{
    ...
    out_draw_data->CmdListsCount = (int)draw_lists->size();
    out_draw_data->TotalVtxCount = out_draw_data->TotalIdxCount = 0;
    out_draw_data->DisplayPos = float2(0.0f, 0.0f);
    out_draw_data->DisplaySize = io.DisplaySize;
    for (int n = 0; n < (int)draw_lists->size(); n++)
    {
        out_draw_data->TotalVtxCount += (int)draw_lists->data()[n]->VtxBuffer.size();
        out_draw_data->TotalIdxCount += (int)draw_lists->data()[n]->IdxBuffer.size();
    }
}

// When using this function it is sane to ensure that float are perfectly rounded to integer values, to that e.g. (int)(max.x-min.x) in user's render produce correct result.
void ImGui::PushClipRect(const float2& clip_rect_min, const float2& clip_rect_max, bool intersect_with_current_clip_rect)
{
    ...
    if (g.ActiveId == 0 && g.HoveredId == 0)
    {
        if (!g.NavWindow || !g.NavWindow->Appearing) // Unless we just made a window/popup appear
        {
            ...
            if (g.IO.MouseClicked[1])
            {
                ...
                for (int i = (int)g.Windows.size() - 1; i >= 0 && hovered_window_above_modal == false; i--)
                {
                    ImGuiWindow* window = g.Windows[i];
                    if (window == modal)
                        break;
...
void ImGui::RenderText(float2 pos, const char* text, const char* text_end, bool hide_text_after_hash)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
void ImGui::RenderTextWrapped(float2 pos, const char* text, const char* text_end, float wrap_width)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
void ImGui::RenderTextClipped(const float2& pos_min, const float2& pos_max, const char* text, const char* text_end, const float2* text_size_if_known, const float2& align, const ImRect* clip_rect)
{
    // Hide anything after a '##' string
    ...
    float2 pos = pos_min;
    const float2 text_size = text_size_if_known ? *text_size_if_known : CalcTextSize(text, text_display_end, false, 0.0f);

    const float2* clip_min = clip_rect ? &clip_rect->Min : &pos_min;
    const float2* clip_max = clip_rect ? &clip_rect->Max : &pos_max;
    ...
    if (need_clipping)
    {
        float4 fine_clip_rect(clip_min->x, clip_min->y, clip_max->x, clip_max->y);
...
void ImGui::RenderFrame(float2 p_min, float2 p_max, uint32_t fill_col, bool border, float rounding)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    {
        window->DrawList->AddRect(p_min+float2(1,1), p_max+float2(1,1), GetColorU32(ImGuiCol_BorderShadow), rounding, ImDrawCornerFlags_All, border_size);
        window->DrawList->AddRect(p_min, p_max, GetColorU32(ImGuiCol_Border), rounding, ImDrawCornerFlags_All, border_size);
    }
}

void ImGui::RenderFrameBorder(float2 p_min, float2 p_max, float rounding)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    const float border_size = g.Style.FrameBorderSize;
...
void ImGui::RenderArrow(float2 p_min, ImGuiDir dir, float scale)
{
    ImGuiContext& g = *GImGui;

    const float h = g.FontSize * 1.00f;
    float r = h * 0.40f * scale;
    float2 center = p_min + float2(h * 0.50f, h * 0.50f * scale);
    ...
    case ImGuiDir_Down:
        ...
        a = float2(+0.000f,+0.750f) * r;
        b = float2(-0.866f,-0.750f) * r;
        c = float2(+0.866f,-0.750f) * r;
        break;
    case ImGuiDir_Left:
    case ImGuiDir_Right:
        if (dir == ImGuiDir_Left) r = -r;
        a = float2(+0.750f,+0.000f) * r;
        b = float2(-0.750f,+0.866f) * r;
        c = float2(-0.750f,-0.866f) * r;
        break;
    case ImGuiDir_None:
    case ImGuiDir_COUNT:
        IM_ASSERT(0);
...
void ImGui::RenderBullet(float2 pos)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
void ImGui::RenderCheckMark(float2 pos, uint32_t col, float sz)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    sz -= thickness*0.5f;
    pos += float2(thickness*0.25f, thickness*0.25f);

    float third = sz / 3.0f;
    float bx = pos.x + third;
    float by = pos.y + sz - third*0.5f;
    window->DrawList->PathLineTo(float2(bx - third, by - third));
    window->DrawList->PathLineTo(float2(bx, by));
    window->DrawList->PathLineTo(float2(bx + third*2, by - third*2));
...
void ImGui::RenderNavHighlight(const ImRect& bb, ImGuiID id, ImGuiNavHighlightFlags flags)
{
    ...
    if (flags & ImGuiNavHighlightFlags_TypeDefault)
    {
        ...
        display_rect.Expand(float2(DISTANCE,DISTANCE));
        bool fully_visible = window->ClipRect.Contains(display_rect);
        if (!fully_visible)
            window->DrawList->PushClipRect(display_rect.Min, display_rect.Max);
        window->DrawList->AddRect(display_rect.Min + float2(THICKNESS*0.5f,THICKNESS*0.5f), display_rect.Max - float2(THICKNESS*0.5f,THICKNESS*0.5f), GetColorU32(ImGuiCol_NavHighlight), rounding, ImDrawCornerFlags_All, THICKNESS);
        if (!fully_visible)
            window->DrawList->PopClipRect();
    }
...
// CalcTextSize("") should return float2(0.0f, GImGui->FontSize)
float2 ImGui::CalcTextSize(const char* text, const char* text_end, bool hide_text_after_double_hash, float wrap_width)
{
    ImGuiContext& g = *GImGui;
    ...
    if (text == text_display_end)
        return float2(0.0f, font_size);
    float2 text_size = font->CalcTextSizeA(font_size, FLT_MAX, wrap_width, text, text_display_end, NULL);
...
void ImGui::CalcListClipping(int items_count, float items_height, int* out_items_display_start, int* out_items_display_end)
{
    ...
    const float2 pos = window->DC.CursorPos;
...
static void FindHoveredWindow()
{
    ...
    for (int i = (int)g.Windows.size() - 1; i >= 0 && hovered_window == NULL; i--)
    {
        ImGuiWindow* window = g.Windows[i];
...
bool ImGui::IsMouseHoveringRect(const float2& r_min, const float2& r_max, bool clip)
{
    ImGuiContext& g = *GImGui;
...
float2 ImGui::GetMousePos()
{
    return GImGui->IO.MousePos;
}

// NB: prefer to call right after BeginPopup(). At the time Selectable/MenuItem is activated, the popup is already closed!
float2 ImGui::GetMousePosOnOpeningCurrentPopup()
{
    ImGuiContext& g = *GImGui;
...
// We typically use float2(-FLT_MAX,-FLT_MAX) to denote an invalid mouse position
bool ImGui::IsMousePosValid(const float2* mouse_pos)
{
    if (mouse_pos == NULL)
...
float2 ImGui::GetMouseDragDelta(int button, float lock_threshold)
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(button >= 0 && button < IM_ARRAYSIZE(g.IO.MouseDown));
    if (lock_threshold < 0.0f)
...
float2 ImGui::GetItemRectMin()
{
    ImGuiWindow* window = GetCurrentWindowRead();
    return window->DC.LastItemRect.Min;
}

float2 ImGui::GetItemRectMax()
{
    ImGuiWindow* window = GetCurrentWindowRead();
    return window->DC.LastItemRect.Max;
}

float2 ImGui::GetItemRectSize()
{
    ImGuiWindow* window = GetCurrentWindowRead();
...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const float2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    ...
    // Size
    const float2 content_avail = GetContentRegionAvail();
    float2 size = ImFloor(size_arg);
...
bool ImGui::BeginChild(const char* str_id, const float2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    return BeginChildEx(str_id, window->GetID(str_id), size_arg, border, extra_flags);
}

bool ImGui::BeginChild(ImGuiID id, const float2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
    IM_ASSERT(id != 0);
    ...
    else
    {
        ...
        if ((window->DC.NavLayerActiveMask != 0 || window->DC.NavHasScroll) && !(window->Flags & ImGuiWindowFlags_NavFlattened))
        {
            ...
            if (window->DC.NavLayerActiveMask == 0 && window == g.NavWindow)
                RenderNavHighlight(ImRect(bb.Min - float2(2,2), bb.Max + float2(2,2)), g.NavId, ImGuiNavHighlightFlags_TypeThin);
        }
        else
        {
            // Not navigable into
...
bool ImGui::BeginChildFrame(ImGuiID id, const float2& size, ImGuiWindowFlags extra_flags)
{
    ImGuiContext& g = *GImGui;
...
static void CheckStacksSize(ImGuiWindow* window, bool write)
{
    ...
    { int current = (int)window->IDStack.size();       if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && "PushID/PopID or TreeNode/TreePop Mismatch!");   p_backup++; }    // Too few or too many PopID()/TreePop()
    { int current = (int)window->DC.GroupStack.size(); if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && "BeginGroup/EndGroup Mismatch!");                p_backup++; }    // Too few or too many EndGroup()
    { int current = (int)g.CurrentPopupStack.size();   if (write) *p_backup = current; else IM_ASSERT(*p_backup == current && "BeginMenu/EndMenu or BeginPopup/EndPopup Mismatch"); p_backup++;}// Too few or too many EndMenu()/EndPopup()
    // For color, style and font stacks there is an incentive to use Push/Begin/Pop/.../End patterns, so we relax our checks a little to allow them.
    { int current = (int)g.ColorModifiers.size();      if (write) *p_backup = current; else IM_ASSERT(*p_backup >= current && "PushStyleColor/PopStyleColor Mismatch!");       p_backup++; }    // Too few or too many PopStyleColor()
    { int current = (int)g.StyleModifiers.size();      if (write) *p_backup = current; else IM_ASSERT(*p_backup >= current && "PushStyleVar/PopStyleVar Mismatch!");           p_backup++; }    // Too few or too many PopStyleVar()
    { int current = (int)g.FontStack.size();           if (write) *p_backup = current; else IM_ASSERT(*p_backup >= current && "PushFont/PopFont Mismatch!");                   p_backup++; }    // Too few or too many PopFont()
...
static float2 CalcSizeAfterConstraint(ImGuiWindow* window, float2 new_size)
{
    ImGuiContext& g = *GImGui;
...
static float2 CalcSizeContents(ImGuiWindow* window)
{
    float2 sz;
...
static float2 CalcSizeAutoFit(ImGuiWindow* window, const float2& size_contents)
{
    ImGuiContext& g = *GImGui;
    ...
    else
    {
        ...
        float2 size_min = style.WindowMinSize;
        if (is_popup || is_menu) // Popups and menus bypass style.WindowMinSize by default, but we give then a non-zero minimum size to facilitate understanding problematic cases (e.g. empty popups)
            size_min = ImMin(size_min, float2(4.0f, 4.0f));
        float2 size_auto_fit = ImClamp(size_contents, size_min, ImMax(size_min, g.IO.DisplaySize - style.DisplaySafeAreaPadding * 2.0f));
        float2 size_auto_fit_after_constraint = CalcSizeAfterConstraint(window, size_auto_fit);
...
float2 ImGui::CalcWindowExpectedSize(ImGuiWindow* window)
{
    float2 size_contents = CalcSizeContents(window);
...
static float2 CalcNextScrollFromScrollTargetAndClamp(ImGuiWindow* window, bool snap_on_edges)
{
    ...
    scroll = ImMax(scroll, float2(0.0f, 0.0f));
...
static void CalcResizePosSizeFromAnyCorner(ImGuiWindow* window, const float2& corner_target, const float2& corner_norm, float2* out_pos, float2* out_size)
{
    float2 pos_min = ImLerp(corner_target, window->Pos, corner_norm);                // Expected window upper-left
    float2 pos_max = ImLerp(window->Pos + window->Size, corner_target, corner_norm); // Expected window lower-right
    float2 size_expected = pos_max - pos_min;
    float2 size_constrained = CalcSizeAfterConstraint(window, size_expected);
    *out_pos = pos_min;
...
{
    { float2(1,1), float2(-1,-1), 0, 3 }, // Lower right
    { float2(0,1), float2(+1,-1), 3, 6 }, // Lower left
    { float2(0,0), float2(+1,+1), 6, 9 }, // Upper left
    { float2(1,0), float2(-1,+1), 9,12 }, // Upper right
};

static ImRect GetBorderRect(ImGuiWindow* window, int border_n, float perp_padding, float thickness)
{
    ImRect rect = window->Rect();
    if (thickness == 0.0f) rect.Max -= float2(1,1);
...
static void ImGui::UpdateManualResize(ImGuiWindow* window, const float2& size_auto_fit, int* border_held, int resize_grip_count, uint32_t resize_grip_col[4])
{
    ImGuiContext& g = *GImGui;
    ...
    float2 pos_target(FLT_MAX, FLT_MAX);
    float2 size_target(FLT_MAX, FLT_MAX);
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++)
    {
        ...
        const float2 corner = ImLerp(window->Pos, window->Pos + window->Size, grip.CornerPos);
        ...
        else if (held)
        {
            ...
            float2 corner_target = g.IO.MousePos - g.ActiveIdClickOffset + resize_rect.GetSize() * grip.CornerPos; // Corner of the window corresponding to our corner grip
    ...
    for (int border_n = 0; border_n < resize_border_count; border_n++)
    {
        ...
        if (held)
        {
            float2 border_target = window->Pos;
            float2 border_posn;
            if (border_n == 0) { border_posn = float2(0, 0); border_target.y = (g.IO.MousePos.y - g.ActiveIdClickOffset.y); }
            if (border_n == 1) { border_posn = float2(1, 0); border_target.x = (g.IO.MousePos.x - g.ActiveIdClickOffset.x + BORDER_SIZE); }
            if (border_n == 2) { border_posn = float2(0, 1); border_target.y = (g.IO.MousePos.y - g.ActiveIdClickOffset.y + BORDER_SIZE); }
            if (border_n == 3) { border_posn = float2(0, 0); border_target.x = (g.IO.MousePos.x - g.ActiveIdClickOffset.x); }
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    {
        float2 size_on_first_use = (g.NextWindowData.SizeCond != 0) ? g.NextWindowData.SizeVal : float2(0.0f, 0.0f); // Any condition flag will do since we are creating a new window here.
    ...
    {
        window->SizeContentsExplicit = float2(0.0f, 0.0f);
    }
    ...
    if (first_begin_of_the_frame)
    {
        ...
        window->BeginCount = 0;
        window->ClipRect = float4(-FLT_MAX,-FLT_MAX,+FLT_MAX,+FLT_MAX);
        ...
        if (window_just_activated_by_user && (flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_Tooltip)) != 0)
        {
            ...
            if (flags & ImGuiWindowFlags_AlwaysAutoResize)
            {
                ...
                window->SizeContents = float2(0.f, 0.f);
            }
        }
        ...
        if ((flags & ImGuiWindowFlags_ChildWindow) && !(flags & (ImGuiWindowFlags_AlwaysUseWindowPadding | ImGuiWindowFlags_Popup)) && window->WindowBorderSize == 0.0f)
            window->WindowPadding = float2(0.0f, (flags & ImGuiWindowFlags_MenuBar) ? style.WindowPadding.y : 0.0f);
        ...
        const float2 size_auto_fit = CalcSizeAutoFit(window, window->SizeContents);
        float2 size_full_modified(FLT_MAX, FLT_MAX);
        ...
        if (!window->Collapsed)
        {
            ...
            window->ScrollbarSizes = float2(window->ScrollbarY ? style.ScrollbarSize : 0.0f, window->ScrollbarX ? style.ScrollbarSize : 0.0f);
        }
        ...
        if (flags & ImGuiWindowFlags_ChildWindow)
        {
            window->BeginOrderWithinParent = (int)parent_window->DC.ChildWindows.size();
        ...
        if (!(flags & ImGuiWindowFlags_ChildWindow))
        {
            ...
            {
                float2 padding = ImMax(style.DisplayWindowPadding, style.DisplaySafeAreaPadding);
        ...
        window->ScrollTarget = float2(FLT_MAX, FLT_MAX);
        ...
        {
            const uint32_t dim_bg_col = GetColorU32(dim_bg_for_modal ? ImGuiCol_ModalWindowDimBg : ImGuiCol_NavWindowingDimBg, g.DimBgRatio);
        ...
        if (window->Collapsed)
        {
            ...
            uint32_t title_bar_col = GetColorU32((title_bar_is_highlight && !g.NavDisableHighlight) ? ImGuiCol_TitleBgActive : ImGuiCol_TitleBgCollapsed);
        ...
        {
            // Window background
            uint32_t bg_col = GetColorU32(GetWindowBgColorIdxFromFlags(flags));
            if (g.NextWindowData.BgAlphaCond != 0)
            {
                bg_col = (bg_col & ~IM_COL32_A_MASK) | (IM_F32_TO_INT8_SAT(g.NextWindowData.BgAlphaVal) << IM_COL32_A_SHIFT);
                g.NextWindowData.BgAlphaCond = 0;
            }
            window->DrawList->AddRectFilled(window->Pos + float2(0.0f, window->TitleBarHeight()), window->Pos + window->Size, bg_col, window_rounding, (flags & ImGuiWindowFlags_NoTitleBar) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Bot);

            // Title bar
            uint32_t title_bar_col = GetColorU32(window->Collapsed ? ImGuiCol_TitleBgCollapsed : title_bar_is_highlight ? ImGuiCol_TitleBgActive : ImGuiCol_TitleBg);
            ...
            if (!(flags & ImGuiWindowFlags_NoResize))
            {
                for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++)
                {
                    const ImGuiResizeGripDef& grip = resize_grip_def[resize_grip_n];
                    const float2 corner = ImLerp(window->Pos, window->Pos + window->Size, grip.CornerPos);
                    window->DrawList->PathLineTo(corner + grip.InnerDir * ((resize_grip_n & 1) ? float2(window_border_size, grip_draw_size) : float2(grip_draw_size, window_border_size)));
                    window->DrawList->PathLineTo(corner + grip.InnerDir * ((resize_grip_n & 1) ? float2(grip_draw_size, window_border_size) : float2(window_border_size, grip_draw_size)));
                    window->DrawList->PathArcToFast(float2(corner.x + grip.InnerDir.x * (window_rounding + window_border_size), corner.y + grip.InnerDir.y * (window_rounding + window_border_size)), window_rounding, grip.AngleMin12, grip.AngleMax12);
            ...
            if (style.FrameBorderSize > 0 && !(flags & ImGuiWindowFlags_NoTitleBar))
                window->DrawList->AddLine(title_bar_rect.GetBL() + float2(style.WindowBorderSize, -1.0f), title_bar_rect.GetBR() + float2(-style.WindowBorderSize, -1.0f), GetColorU32(ImGuiCol_Border), style.FrameBorderSize);
        }
        ...
        window->DC.CursorStartPos = window->Pos + float2(window->DC.Indent.x + window->DC.ColumnsOffset.x, window->TitleBarHeight() + window->MenuBarHeight() + window->WindowPadding.y - window->Scroll.y);
        window->DC.CursorPos = window->DC.CursorStartPos;
        window->DC.CursorPosPrevLine = window->DC.CursorPos;
        window->DC.CursorMaxPos = window->DC.CursorStartPos;
        window->DC.CurrentLineSize = window->DC.PrevLineSize = float2(0.0f, 0.0f);
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar))
        {
            ...
            if (p_open != NULL)
            {
                ...
                if (CloseButton(window->GetID("#CLOSE"), window->Rect().GetTR() + float2(-pad - rad, pad + rad), rad + 1))
                    *p_open = false;
            }
            ...
            float2 text_size = CalcTextSize(name, NULL, true);
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
bool ImGui::Begin(const char* name, bool* p_open, const float2& size_first_use, float bg_alpha_override, ImGuiWindowFlags flags)
{
...
void ImGui::BringWindowToFront(ImGuiWindow* window)
{
    ...
    if (current_front_window == window || current_front_window->RootWindow == window)
        return;
    for (int i = (int)g.Windows.size() - 2; i >= 0; i--) // We can ignore the front most window
...
// FIXME: This may incur a round-trip (if the end user got their data from a float4) but eventually we aim to store the in-flight colors as uint32_t
void ImGui::PushStyleColor(ImGuiCol idx, uint32_t col)
{
    ImGuiContext& g = *GImGui;
...
void ImGui::PushStyleColor(ImGuiCol idx, const float4& col)
{
    ImGuiContext& g = *GImGui;
    ImGuiColorMod backup;
    backup.Col = idx;
...
{
    { ImGuiDataType_Float, 1, (uint32_t)IM_OFFSETOF(ImGuiStyle, Alpha) },              // ImGuiStyleVar_Alpha
    { ImGuiDataType_Float, 2, (uint32_t)IM_OFFSETOF(ImGuiStyle, WindowPadding) },      // ImGuiStyleVar_WindowPadding
    { ImGuiDataType_Float, 1, (uint32_t)IM_OFFSETOF(ImGuiStyle, WindowRounding) },     // ImGuiStyleVar_WindowRounding
    { ImGuiDataType_Float, 1, (uint32_t)IM_OFFSETOF(ImGuiStyle, WindowBorderSize) },   // ImGuiStyleVar_WindowBorderSize
    { ImGuiDataType_Float, 2, (uint32_t)IM_OFFSETOF(ImGuiStyle, WindowMinSize) },      // ImGuiStyleVar_WindowMinSize
    { ImGuiDataType_Float, 2, (uint32_t)IM_OFFSETOF(ImGuiStyle, WindowTitleAlign) },   // ImGuiStyleVar_WindowTitleAlign
    { ImGuiDataType_Float, 1, (uint32_t)IM_OFFSETOF(ImGuiStyle, ChildRounding) },      // ImGuiStyleVar_ChildRounding
    { ImGuiDataType_Float, 1, (uint32_t)IM_OFFSETOF(ImGuiStyle, ChildBorderSize) },    // ImGuiStyleVar_ChildBorderSize
    { ImGuiDataType_Float, 1, (uint32_t)IM_OFFSETOF(ImGuiStyle, PopupRounding) },      // ImGuiStyleVar_PopupRounding
    { ImGuiDataType_Float, 1, (uint32_t)IM_OFFSETOF(ImGuiStyle, PopupBorderSize) },    // ImGuiStyleVar_PopupBorderSize
    { ImGuiDataType_Float, 2, (uint32_t)IM_OFFSETOF(ImGuiStyle, FramePadding) },       // ImGuiStyleVar_FramePadding
    { ImGuiDataType_Float, 1, (uint32_t)IM_OFFSETOF(ImGuiStyle, FrameRounding) },      // ImGuiStyleVar_FrameRounding
    { ImGuiDataType_Float, 1, (uint32_t)IM_OFFSETOF(ImGuiStyle, FrameBorderSize) },    // ImGuiStyleVar_FrameBorderSize
    { ImGuiDataType_Float, 2, (uint32_t)IM_OFFSETOF(ImGuiStyle, ItemSpacing) },        // ImGuiStyleVar_ItemSpacing
    { ImGuiDataType_Float, 2, (uint32_t)IM_OFFSETOF(ImGuiStyle, ItemInnerSpacing) },   // ImGuiStyleVar_ItemInnerSpacing
    { ImGuiDataType_Float, 1, (uint32_t)IM_OFFSETOF(ImGuiStyle, IndentSpacing) },      // ImGuiStyleVar_IndentSpacing
    { ImGuiDataType_Float, 1, (uint32_t)IM_OFFSETOF(ImGuiStyle, ScrollbarSize) },      // ImGuiStyleVar_ScrollbarSize
    { ImGuiDataType_Float, 1, (uint32_t)IM_OFFSETOF(ImGuiStyle, ScrollbarRounding) },  // ImGuiStyleVar_ScrollbarRounding
    { ImGuiDataType_Float, 1, (uint32_t)IM_OFFSETOF(ImGuiStyle, GrabMinSize) },        // ImGuiStyleVar_GrabMinSize
    { ImGuiDataType_Float, 1, (uint32_t)IM_OFFSETOF(ImGuiStyle, GrabRounding) },       // ImGuiStyleVar_GrabRounding
    { ImGuiDataType_Float, 2, (uint32_t)IM_OFFSETOF(ImGuiStyle, ButtonTextAlign) },    // ImGuiStyleVar_ButtonTextAlign
};
...
void ImGui::PushStyleVar(ImGuiStyleVar idx, const float2& val)
{
    const ImGuiStyleVarInfo* var_info = GetStyleVarInfo(idx);
    if (var_info->Type == ImGuiDataType_Float && var_info->Count == 2)
    {
        ImGuiContext& g = *GImGui;
        float2* pvar = (float2*)var_info->GetVarPtr(&g.Style);
        g.StyleModifiers.push_back(ImGuiStyleMod(idx, *pvar));
        *pvar = val;
        return;
    }
    IM_ASSERT(0); // Called function with wrong-type? Variable is not a float2.
}
...
static void SetWindowPos(ImGuiWindow* window, const float2& pos, ImGuiCond cond)
{
    ...
    window->SetWindowPosAllowFlags &= ~(ImGuiCond_Once | ImGuiCond_FirstUseEver | ImGuiCond_Appearing);
    window->SetWindowPosVal = float2(FLT_MAX, FLT_MAX);

    // Set
    const float2 old_pos = window->Pos;
    window->Pos = ImFloor(pos);
    window->DC.CursorPos += (window->Pos - old_pos);    // As we happen to move the window while it is being appended to (which is a bad idea - will smear) let's at least offset the cursor
    window->DC.CursorMaxPos += (window->Pos - old_pos); // And more importantly we need to adjust this so size calculation doesn't get affected.
}

void ImGui::SetWindowPos(const float2& pos, ImGuiCond cond)
{
    ImGuiWindow* window = GetCurrentWindowRead();
    SetWindowPos(window, pos, cond);
}

void ImGui::SetWindowPos(const char* name, const float2& pos, ImGuiCond cond)
{
    if (ImGuiWindow* window = FindWindowByName(name))
        SetWindowPos(window, pos, cond);
}

float2 ImGui::GetWindowSize()
{
    ImGuiWindow* window = GetCurrentWindowRead();
    return window->Size;
}
...
void ImGui::SetWindowSize(const float2& size, ImGuiCond cond)
{
    SetWindowSize(GImGui->CurrentWindow, size, cond);
}

void ImGui::SetWindowSize(const char* name, const float2& size, ImGuiCond cond)
{
    if (ImGuiWindow* window = FindWindowByName(name))
...
void ImGui::SetNextWindowPos(const float2& pos, ImGuiCond cond, const float2& pivot)
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(cond == 0 || ImIsPowerOfTwo(cond)); // Make sure the user doesn't attempt to combine multiple condition flags.
...
void ImGui::SetNextWindowSize(const float2& size, ImGuiCond cond)
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(cond == 0 || ImIsPowerOfTwo(cond)); // Make sure the user doesn't attempt to combine multiple condition flags.
...
void ImGui::SetNextWindowSizeConstraints(const float2& size_min, const float2& size_max, ImGuiSizeCallback custom_callback, void* custom_callback_user_data)
{
    ImGuiContext& g = *GImGui;
...
void ImGui::SetNextWindowContentSize(const float2& size)
{
    ImGuiContext& g = *GImGui;
...
float2 ImGui::GetContentRegionMax()
{
    ImGuiWindow* window = GetCurrentWindowRead();
    float2 mx = window->ContentsRegionRect.Max - window->Pos;
    if (window->DC.ColumnsSet)
        mx.x = GetColumnOffset(window->DC.ColumnsSet->Current + 1) - window->WindowPadding.x;
    return mx;
}

float2 ImGui::GetContentRegionAvail()
{
    ImGuiWindow* window = GetCurrentWindowRead();
...
// In window space (not screen space!)
float2 ImGui::GetWindowContentRegionMin()
{
    ImGuiWindow* window = GetCurrentWindowRead();
    return window->ContentsRegionRect.Min - window->Pos;
}

float2 ImGui::GetWindowContentRegionMax()
{
    ImGuiWindow* window = GetCurrentWindowRead();
...
float2 ImGui::GetFontTexUvWhitePixel()
{
...
void ImGui::SetCursorPos(const float2& local_pos)
{
    ImGuiWindow* window = GetCurrentWindow();
...
float2 ImGui::GetCursorStartPos()
{
    ImGuiWindow* window = GetCurrentWindowRead();
    return window->DC.CursorStartPos - window->Pos;
}

float2 ImGui::GetCursorScreenPos()
{
    ImGuiWindow* window = GetCurrentWindowRead();
    return window->DC.CursorPos;
}

void ImGui::SetCursorScreenPos(const float2& screen_pos)
{
    ImGuiWindow* window = GetCurrentWindow();
...
void ImGui::Separator()
{
    ...
    const ImRect bb(float2(x1, window->DC.CursorPos.y), float2(x2, window->DC.CursorPos.y+1.0f));
    ItemSize(float2(0.0f, 0.0f)); // NB: we don't provide our width so that it doesn't get feed back into AutoFit, we don't provide height to not alter layout.
    if (!ItemAdd(bb, 0))
    {
        if (window->DC.ColumnsSet)
            PushColumnClipRect();
    ...
    window->DrawList->AddLine(bb.Min, float2(bb.Max.x,bb.Min.y), GetColorU32(ImGuiCol_Separator));
...
void ImGui::VerticalSeparator()
{
    ...
    const ImRect bb(float2(window->DC.CursorPos.x, y1), float2(window->DC.CursorPos.x + 1.0f, y2));
    ItemSize(float2(bb.GetWidth(), 0.0f));
    if (!ItemAdd(bb, 0))
        return;
...
bool ImGui::SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend, float hover_visibility_delay)
{
    ...
    bb_interact.Expand(axis == ImGuiAxis_Y ? float2(0.0f, hover_extend) : float2(hover_extend, 0.0f));
    ...
    if (held)
    {
        float2 mouse_delta_2d = g.IO.MousePos - g.ActiveIdClickOffset - bb_interact.Min;
        ...
        if (mouse_delta != 0.0f)
        {
            ...
            *size1 += mouse_delta;
            *size2 -= mouse_delta;
            bb_render.Translate((axis == ImGuiAxis_X) ? float2(mouse_delta, 0.0f) : float2(0.0f, mouse_delta));
            MarkItemEdited(id);
        }
    }

    // Render
    const uint32_t col = GetColorU32(held ? ImGuiCol_SeparatorActive : (hovered && g.HoveredIdTimer >= hover_visibility_delay) ? ImGuiCol_SeparatorHovered : ImGuiCol_Separator);
...
void ImGui::Dummy(const float2& size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
...
bool ImGui::IsRectVisible(const float2& size)
{
    ImGuiWindow* window = GetCurrentWindowRead();
    return window->ClipRect.Overlaps(ImRect(window->DC.CursorPos, window->DC.CursorPos + size));
}

bool ImGui::IsRectVisible(const float2& rect_min, const float2& rect_max)
{
    ImGuiWindow* window = GetCurrentWindowRead();
...
void ImGui::BeginGroup()
{
    ...
    window->DC.CurrentLineSize = float2(0.0f, 0.0f);
...
void ImGui::NewLine()
{
    ...
    if (window->DC.CurrentLineSize.y > 0.0f)     // In the event that we are on a line with items that is smaller that FontSize high, we will preserve its height.
        ...
        ItemSize(float2(0.0f, g.FontSize));
...
void ImGui::BeginTooltip()
{
    ...
    if (g.DragDropWithinSourceOrTarget)
    {
        ...
        //float2 tooltip_pos = g.IO.MousePos - g.ActiveIdClickOffset - g.Style.WindowPadding;
        float2 tooltip_pos = g.IO.MousePos + float2(16 * g.Style.MouseCursorScale, 8 * g.Style.MouseCursorScale);
...
{
    ImGuiContext& g = *GImGui;
    for (int n = (int)g.OpenPopupStack.size()-1; n >= 0; n--)
...
void ImGui::OpenPopupEx(ImGuiID id)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* parent_window = g.CurrentWindow;
    int current_stack_size = (int)g.CurrentPopupStack.size();
...
void ImGui::ClosePopup(ImGuiID id)
{
    if (!IsPopupOpen(id))
        return;
    ImGuiContext& g = *GImGui;
    ClosePopupToLevel((int)g.OpenPopupStack.size() - 1);
}
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags)
{
    ...
    if (extra_flags & ImGuiWindowFlags_ChildMenu)
        ImFormatString(name, IM_ARRAYSIZE(name), "##Menu_%02d", (int)g.CurrentPopupStack.size());    // Recycle windows based on depth
    else
...
bool ImGui::BeginPopupModal(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (g.NextWindowData.PosCond == 0)
        SetNextWindowPos(g.IO.DisplaySize * 0.5f, ImGuiCond_Appearing, float2(0.5f, 0.5f));
...
{
    float2 padding = GImGui->Style.DisplaySafeAreaPadding;
    ImRect r_screen = GetViewportRect();
    r_screen.Expand(float2((r_screen.GetWidth() > padding.x * 2) ? -padding.x : 0.0f, (r_screen.GetHeight() > padding.y * 2) ? -padding.y : 0.0f));
    return r_screen;
}

// r_avoid = the rectangle to avoid (e.g. for tooltip it is a rectangle around the mouse cursor which we want to avoid. for popups it's a small point around the cursor.)
// r_outer = the visible area rectangle, minus safe area padding. If our popup size won't fit because of safe area padding we ignore it.
float2 ImGui::FindBestWindowPosForPopupEx(const float2& ref_pos, const float2& size, ImGuiDir* last_dir, const ImRect& r_outer, const ImRect& r_avoid, ImGuiPopupPositionPolicy policy)
{
    float2 base_pos_clamped = ImClamp(ref_pos, r_outer.Min, r_outer.Max - size);
    ...
    if (policy == ImGuiPopupPositionPolicy_ComboBox)
    {
        ...
        for (int n = (*last_dir != ImGuiDir_None) ? -1 : 0; n < ImGuiDir_COUNT; n++)
        {
            ...
            if (n != -1 && dir == *last_dir) // Already tried this direction?
                continue;
            float2 pos;
            if (dir == ImGuiDir_Down)  pos = float2(r_avoid.Min.x, r_avoid.Max.y);          // Below, Toward Right (default)
            if (dir == ImGuiDir_Right) pos = float2(r_avoid.Min.x, r_avoid.Min.y - size.y); // Above, Toward Right
            if (dir == ImGuiDir_Left)  pos = float2(r_avoid.Max.x - size.x, r_avoid.Max.y); // Below, Toward Left
            if (dir == ImGuiDir_Up)    pos = float2(r_avoid.Max.x - size.x, r_avoid.Min.y - size.y); // Above, Toward Left
...
float2 ImGui::FindBestWindowPosForPopup(ImGuiWindow* window)
{
    ImGuiContext& g = *GImGui;
    ...
    if (window->Flags & ImGuiWindowFlags_Tooltip)
    {
        ...
        float2 ref_pos = NavCalcPreferredRefPos();
        ImRect r_avoid;
        if (!g.NavDisableHighlight && g.NavDisableMouseHover && !(g.IO.ConfigFlags & ImGuiConfigFlags_NavEnableSetMousePos))
            r_avoid = ImRect(ref_pos.x - 16, ref_pos.y - 8, ref_pos.x + 16, ref_pos.y + 8);
        else
            r_avoid = ImRect(ref_pos.x - 16, ref_pos.y - 8, ref_pos.x + 24 * sc, ref_pos.y + 24 * sc); // FIXME: Hard-coded based on mouse cursor shape expectation. Exact dimension not very important.
        float2 pos = FindBestWindowPosForPopupEx(ref_pos, window->Size, &window->AutoPosLastDirection, r_outer, r_avoid);
        if (window->AutoPosLastDirection == ImGuiDir_None)
            pos = ref_pos + float2(2, 2); // If there's not enough room, for tooltip we prefer avoiding the cursor at all cost even if it means that part of the tooltip won't be visible.
        return pos;
    }
    IM_ASSERT(0);
    return window->Pos;
...
static bool NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    if (ImGui::IsMouseHoveringRect(cand.Min, cand.Max))
    {
        ...
        draw_list->AddRectFilled(cand.Max-float2(4,4), cand.Max+ImGui::CalcTextSize(buf)+float2(4,4), IM_COL32(40,0,0,150));
...
static float2 ImGui::NavCalcPreferredRefPos()
{
    ImGuiContext& g = *GImGui;
    ...
    float2 pos = g.NavWindow->Pos + float2(rect_rel.Min.x + ImMin(g.Style.FramePadding.x*4, rect_rel.GetWidth()), rect_rel.Max.y - ImMin(g.Style.FramePadding.y, rect_rel.GetHeight()));
...
float2 ImGui::GetNavInputAmount2d(ImGuiNavDirSourceFlags dir_sources, ImGuiInputReadMode mode, float slow_factor, float fast_factor)
{
    float2 delta(0.0f, 0.0f);
    if (dir_sources & ImGuiNavDirSourceFlags_Keyboard)
        delta += float2(GetNavInputAmount(ImGuiNavInput_KeyRight_, mode)   - GetNavInputAmount(ImGuiNavInput_KeyLeft_,   mode), GetNavInputAmount(ImGuiNavInput_KeyDown_,   mode) - GetNavInputAmount(ImGuiNavInput_KeyUp_,   mode));
    if (dir_sources & ImGuiNavDirSourceFlags_PadDPad)
        delta += float2(GetNavInputAmount(ImGuiNavInput_DpadRight, mode)   - GetNavInputAmount(ImGuiNavInput_DpadLeft,   mode), GetNavInputAmount(ImGuiNavInput_DpadDown,   mode) - GetNavInputAmount(ImGuiNavInput_DpadUp,   mode));
    if (dir_sources & ImGuiNavDirSourceFlags_PadLStick)
        delta += float2(GetNavInputAmount(ImGuiNavInput_LStickRight, mode) - GetNavInputAmount(ImGuiNavInput_LStickLeft, mode), GetNavInputAmount(ImGuiNavInput_LStickDown, mode) - GetNavInputAmount(ImGuiNavInput_LStickUp, mode));
...
{
    ImRect window_rect(window->InnerMainRect.Min - float2(1, 1), window->InnerMainRect.Max + float2(1, 1));
...
static void ImGui::NavUpdate()
{
    ...
    if (g.NavWindow && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) && !g.NavWindowingTarget)
    {
        ...
        float2 scroll_dir = GetNavInputAmount2d(ImGuiNavDirSourceFlags_PadLStick, ImGuiInputReadMode_Down, 1.0f/10.0f, 10.0f);
    ...
    {
        ImGuiWindow* window = g.NavWindow;
        ImRect window_rect_rel(window->InnerMainRect.Min - window->Pos - float2(1,1), window->InnerMainRect.Max - window->Pos + float2(1,1));
        if (!window_rect_rel.Contains(window->NavRectRel[g.NavLayer]))
        {
            float pad = window->CalcFontSize() * 0.5f;
            window_rect_rel.Expand(float2(-ImMin(window_rect_rel.GetWidth(), pad), -ImMin(window_rect_rel.GetHeight(), pad))); // Terrible approximation for the intent of starting navigation from first fully visible item
    ...
    if (g.NavWindow) { uint32_t col = (g.NavWindow->HiddenFrames == 0) ? IM_COL32(255,0,255,255) : IM_COL32(255,0,0,255); float2 p = NavCalcPreferredRefPos(); char buf[32]; ImFormatString(buf, 32, "%d", g.NavLayer); g.OverlayDrawList.AddCircleFilled(p, 3.0f, col); g.OverlayDrawList.AddText(NULL, 13.0f, p + float2(8,-4), col, buf); }
#endif
}
...
static void ImGui::NavUpdateMoveResult()
{
    ...
    if (g.NavLayer == 0)
    {
        ...
        float2 next_scroll = CalcNextScrollFromScrollTargetAndClamp(result->Window, false);
        float2 delta_scroll = result->Window->Scroll - next_scroll;
...
static void NavUpdateWindowingHighlightWindow(int focus_change_dir)
{
    ...
    if (!window_target)
        window_target = FindWindowNavFocusable((focus_change_dir < 0) ? ((int)g.Windows.size() - 1) : 0, i_current, focus_change_dir);
...
static void ImGui::NavUpdateWindowing()
{
    ...
    if (start_windowing_with_gamepad || start_windowing_with_keyboard)
        if (ImGuiWindow* window = g.NavWindow ? g.NavWindow : FindWindowNavFocusable((int)g.Windows.size() - 1, -INT_MAX, -1))
        {
...
void ImGui::NavUpdateWindowingList()
{
    ...
    SetNextWindowSizeConstraints(float2(g.IO.DisplaySize.x * 0.20f, g.IO.DisplaySize.y * 0.20f), float2(FLT_MAX, FLT_MAX));
    SetNextWindowPos(g.IO.DisplaySize * 0.5f, ImGuiCond_Always, float2(0.5f, 0.5f));
    ...
    for (int n = (int)g.Windows.size() - 1; n >= 0; n--)
    {
        ImGuiWindow* window = g.Windows[n];
...
void ImGui::EndColumns()
{
    ...
    if (!(columns->Flags & ImGuiColumnsFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (int n = 1; n < columns->Count; n++)
        {
            ...
            const ImRect column_rect(float2(x - column_hw, y1), float2(x + column_hw, y2));
            ...
            const uint32_t col = GetColorU32(held ? ImGuiCol_SeparatorActive : hovered ? ImGuiCol_SeparatorHovered : ImGuiCol_Separator);
            const float xi = (float)(int)x;
            window->DrawList->AddLine(float2(xi, ImMax(y1 + 1.0f, window->ClipRect.Min.y)), float2(xi, ImMin(y2, window->ClipRect.Max.y)), col);
        }
...
const ImGuiPayload* ImGui::AcceptDragDropPayload(const char* type, ImGuiDragDropFlags flags)
{
    ...
    if (!(flags & ImGuiDragDropFlags_AcceptNoDrawDefaultRect) && payload.Preview)
    {
        ...
        if (push_clip_rect) window->DrawList->PushClipRect(r.Min-float2(1,1), r.Max+float2(1,1));
...
void ImGui::LogRenderedText(const float2* ref_pos, const char* text, const char* text_end)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
static void SettingsHandlerWindow_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line)
{
    ...
    float x, y;
    int i;
    if (sscanf(line, "Pos=%f,%f", &x, &y) == 2)         settings->Pos = float2(x, y);
    else if (sscanf(line, "Size=%f,%f", &x, &y) == 2)   settings->Size = ImMax(float2(x, y), GImGui->Style.WindowMinSize);
...
static void SettingsHandlerWindow_WriteAll(ImGuiContext* imgui_ctx, ImGuiSettingsHandler* handler, eastl::string* buf)
{
    ...
    buf->reserve(buf->size() + (int)g.SettingsWindows.size() * 96); // ballpark reserve
    for (int i = 0; i != g.SettingsWindows.size(); i++)
    {
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    if (ImGui::Begin("ImGui Metrics", p_open))
    {
        ...
        struct Funcs
        {
            ...
            {
                bool node_open = ImGui::TreeNode(draw_list, "%s: '%s' %d vtx, %d indices, %d cmds", label, draw_list->_OwnerName ? draw_list->_OwnerName : "", (int)draw_list->VtxBuffer.size(), (int)draw_list->IdxBuffer.size(), (int)draw_list->CmdBuffer.size());
            ...
            static void NodeWindow(ImGuiWindow* window, const char* label)
            {
                ...
                if (window->ColumnsStorage.size() > 0 && ImGui::TreeNode("Columns", "Columns sets (%d)", (int)window->ColumnsStorage.size()))
                {
                    for (int n = 0; n < window->ColumnsStorage.size(); n++)
                    {
                ...
                ImGui::BulletText("Storage: %d bytes", (int)window->StateStorage.Data.size() * (int)sizeof(ImGuiStorage::Pair));
                ImGui::TreePop();
            }
        };
        ...
        if (ImGui::TreeNode("DrawList", "Active DrawLists (%d)", (int)g.DrawDataBuilder.Layers[0].size()))
        {
            for (int i = 0; i < g.DrawDataBuilder.Layers[0].size(); i++)
                Funcs::NodeDrawList(NULL, g.DrawDataBuilder.Layers[0][i], "DrawList");
            ImGui::TreePop();
        }
        if (ImGui::TreeNode("Popups", "Popups (%d)", (int)g.OpenPopupStack.size()))
        {
        ...
        if (g.IO.KeyCtrl && show_window_begin_order)
        {
            for (int n = 0; n < g.Windows.size(); n++)
            {
                ...
                overlay_draw_list->AddRectFilled(window->Pos, window->Pos + float2(font_size, font_size), IM_COL32(200, 100, 100, 255));
...
#include "Middleware/UI/imgui_user.cpp"
#endif
